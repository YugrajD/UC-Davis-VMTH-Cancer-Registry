    ...
    *,
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch in pbar:
            ...
            report_hidden = model.base_model(
                input_ids=report_input_ids,
                attention_mask=report_attention_mask,
            ).last_hidden_state

            label_hidden = model.base_model(
                input_ids=label_input_ids,
                attention_mask=label_attention_mask,
            ).last_hidden_state

            report_emb = _mean_pool(report_hidden, report_attention_mask)
            label_emb  = _mean_pool(label_hidden,  label_attention_mask)
