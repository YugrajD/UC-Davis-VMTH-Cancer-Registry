#!/usr/bin/env python3
"""
Parse all per-year CSVs in new_data/ into three output files:
  - demographics.csv
  - diagnoses.csv
  - report.csv
"""
...
OUTPUT_DIR = os.path.join(os.path.dirname(__file__), 'new_data', 'output')
...
def write_outputs(output_dir, parsed_cases):
    """Write demographics.csv, diagnoses.csv, and report.csv into output_dir."""
    ...
    diag_count = sum(len(pc['diagnoses']) for pc in parsed_cases)
    return len(parsed_cases), diag_count, len(report_columns)


def main():
    input_files = sorted(glob.glob(os.path.join(DATA_DIR, '*.csv')))
    if not input_files:
    ...
    for path in input_files:
        ...
        for case in raw_cases:
            ...
            parsed_cases.append({
                'case_id': f'{year}-{case_number:04d}',
                'case': case,
                'diagnoses': diagnoses,
                'sections': sections,
            })
        ...
        n_cases, n_diag, n_cols = write_outputs(year_dir, parsed_cases)
        print(f"  {year}: {len(raw_cases)} raw, {n_cases} written, {skipped} skipped, {n_diag} diagnoses")

        total_cases += n_cases
        total_skipped += skipped
        total_diagnoses += n_diag

    print(f"\nTotal: {total_cases} cases, {total_diagnoses} diagnoses across {len(input_files)} year(s)")
    print(f"Skipped {total_skipped} cases with no report text")
    print(f"Output written to {OUTPUT_DIR}/<year>/")
