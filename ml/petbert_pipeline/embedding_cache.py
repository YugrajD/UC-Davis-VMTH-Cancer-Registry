...
def save_cache(
    ...
    *,
    ...
    report_csv_path: str,
    labels_csv_path: str,
    enriched_label_embeddings: np.ndarray | None = None,
) -> None:
    ...
    if enriched_label_embeddings is not None:
        arrays["enriched_label_embeddings"] = enriched_label_embeddings
    for col in col_names:
        s = _safe(col)
        arrays[f"col_{s}"] = col_embeddings[col]
        arrays[f"has_{s}"] = col_has_content[col]
    np.savez(path, **arrays)
    enriched_note = " (with enriched labels)" if enriched_label_embeddings is not None else ""
    print(f"Saved embedding cache ({len(case_ids)} cases, {len(label_texts)} labels{enriched_note}): {path}")
...
def load_cache(
    ...
    *,
    ...
    If ``require_enriched`` is True and the cache does not contain
    ``enriched_label_embeddings``, the cache is treated as a miss so that it
    gets rebuilt with enrichment enabled.
    """
    ...
    try:
        ...
        enriched = (
            data["enriched_label_embeddings"]
            if "enriched_label_embeddings" in data
            else None
        )
