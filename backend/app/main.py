...
async def lifespan(app: FastAPI):
    ...
    # Rebuild materialized views to count only top-1 predictions (migration 028).
    try:
        async with async_session() as db:
            row = await db.execute(text(
                "SELECT 1 FROM pg_matviews "
                "WHERE matviewname = 'mv_county_cancer_incidence' "
                "AND definition LIKE '%diagnosis_index%'"
            ))
            if row.scalar() is None:
                logger.info("Applying migration 028: rebuilding materialized views for top-1 only")
                await db.execute(text(
                    "DROP MATERIALIZED VIEW IF EXISTS mv_county_cancer_incidence CASCADE"
                ))
                await db.execute(text("""
                ...
                await db.commit()
                logger.info("Migration 028 applied: materialized views rebuilt for top-1 only")
    except Exception as e:
