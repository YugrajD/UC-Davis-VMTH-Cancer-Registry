...
// Floci's app client (created by floci-init.sh) only allows
// USER_PASSWORD_AUTH (no SRP support), while the real Cognito pool should
// keep using the SDK's default USER_SRP_AUTH so the password never leaves
...
// client are configured on the real Cognito User Pool — Floci's Cognito
// emulation has no federation support, so this can't be exercised locally.
